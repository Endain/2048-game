        ...
        for (var y = 0; y < this.cy; y++) {
            for (var x = 1; x < this.cx; x++) {
                if (!this.grid[x - 1][y] || !this.grid[x][y]) 
                    return false;
                if (this.grid[x - 1][y].getValue() === this.grid[x][y].getValue()) 
                    return false;
                }
            }
        ...
        for (var x = 0; x < this.cx; x++) {
            for (var y = 1; y < this.cy; y++) {
                if (!this.grid[x][y - 1] || !this.grid[x][y]) 
                    return false;
                if (this.grid[x][y - 1].getValue() === this.grid[x][y].getValue()) 
                    return false;
                }
            }
    ...
    Tile.prototype.setElement = function (element) {
        ...
        this.element.addEventListener('transitionend', (function (event) {
            ...
            if (event.propertyName === 'left' || event.propertyName === 'top') {
                ...
                if (this.delete) 
                    this.game.target.removeChild(this.element);
                }
            }).bind(this));
    }
